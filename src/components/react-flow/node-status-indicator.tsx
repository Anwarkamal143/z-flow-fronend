...
export const SpinnerLoadingIndicator = ({
  ...
  return (
    <div className='relative'>
      <StatusBorder className='text-nd-loading/40'>{children}</StatusBorder>

      <div className='bg-background/50 absolute inset-0 z-50 rounded-[9px] backdrop-blur-xs' />
      <div className='absolute inset-0 z-50'>
        <span className='text-nd-loading/20 absolute top-[calc(50%-1.25rem)] left-[calc(50%-1.25rem)] inline-block h-10 w-10 animate-ping rounded-full' />

        <LoaderCircle className='text-nd-loading absolute top-[calc(50%-0.75rem)] left-[calc(50%-0.75rem)] size-6 animate-spin' />
      </div>
    </div>
  )
}
    ...
    <>
      ...
      `}
        ...
        >
          ...
          <div className='spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,var(--color-nd-loading)_0deg,rgba(42,138,246,0)_360deg)]' />
        </div>
      </div>
      {children}
    </>
